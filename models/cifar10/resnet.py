...
def adapt_channel(compress_rate, num_layers):
    ...
    stage_oup_cprate = []
    stage_oup_cprate += [compress_rate[0]] #처음 element 는 (0임) 첫번째 conv 의 pruning ratio
    for i in range(len(stage_repeat)-1): #2번째, 3번째 cprate는 첫번째 layer, 두번째 layer에 공통된 pruning ratio임
        stage_oup_cprate += [compress_rate[i+1]] * stage_repeat[i]
    stage_oup_cprate +=[0.] * stage_repeat[-1]
    mid_cprate = compress_rate[len(stage_repeat):] #첫 3개 이후에 4번째부터 두 conv 사이의 중간 pruning ratio
    ...
    return overall_channel, mid_channel #두개의 conv가 연이어 나올경우에 mid는 중간에 줄어드는 channel, overall은 맨 앞쪽과 뒤쪽의 채널
